...
RSpec.describe PredictabilityEngine::VegaVisualizer do
  ...
  let(:items) do
    [
      PredictabilityEngine::Models::WorkItem.new(item_id: 'T1', type: 'Task', title: 'T1', start_date: today - 10, end_date: today - 5),
      PredictabilityEngine::Models::WorkItem.new(item_id: 'T2', type: 'Task', title: 'T2', start_date: today - 5, end_date: nil)
    ]
  end

  around do |example|
    old_mock_today = ENV['MOCK_TODAY']
  ...
  describe '.forecasted_cfd' do
    ...
    it 'includes vertical forecast layers with date-enhanced labels' do
      ...
      label_data = data_values.find { |v| v['label'] && v['label'].include?('%') }
