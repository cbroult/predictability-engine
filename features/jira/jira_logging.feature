Feature: Jira HTTP debug logging
  ...
  # JIRA_PROJECT must be set so the data source resolves a query and actually
  # issues an HTTP request — without it the code raises before any call is made.
  ...
  Background:
    ...
    And the environment variable "JIRA_PROJECT" is set to "TESTPROJECT"

  Scenario: JIRA_HTTP_DEBUG=true prints the HTTP method and path before each Jira request
    Given the environment variable "JIRA_HTTP_DEBUG" is set to "true"
