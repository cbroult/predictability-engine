...
3. `.woodpecker/verify-windows.yml` — exists; `depends_on: [publish]`; has `CBP_ORG_CA_CERT`, `FORGEJO_API_TOKEN` in environment
...
7. `scripts/download-gem-artifact.sh` — exists; downloads gem from Forgejo generic packages via `forgejo_api_token`
