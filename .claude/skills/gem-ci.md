...
3. `.woodpecker/verify-windows.yml` — exists; `depends_on: [publish]`; has `CBP_ORG_CA_CERT`, `FORGEJO_PUSH_TOKEN` in environment
...
6. `scripts/upload-gem-artifact.sh` — exists; uploads gem to Forgejo generic packages via curl PUT; does NOT push to any gem registry
7. `scripts/download-gem-artifact.sh` — exists; downloads gem from Forgejo generic packages via `forgejo_push_token`
