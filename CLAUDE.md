...
- **Commit and push regularly**: commit after each logical unit of work is complete and `bundle exec rake verify` passes, not only at the very end. Use Conventional Commit messages. Push after each commit (or after a version-bump commit immediately following). Do not batch up multiple unrelated changes into one commit, and do not wait to be asked. **Always push to both remotes**: `git push origin` then `git push github` — never push to only one.
- **Version bump on every push**: before pushing, run `bundle exec rake version:bump[type]` choosing the level based on the highest-impact change in the batch — `patch` for bug-fixes only, `minor` for any new backward-compatible feature, `major` for breaking changes. Commit the version bump as a separate `chore: bump version to X.Y.Z` commit immediately before the push.
- **Fail-fast**: surface descriptive errors; do not silently swallow exceptions in calculators or simulators.
- **Never `logger.error` + `exit` inside methods**: only the outermost caller (Thor CLI entry point) decides whether to exit. Inner methods must raise an exception and let the caller handle it. Do not add `rescue … logger.error { e.message }; exit 1` blocks inside non-CLI methods — that pattern silently swallows stack context and robs the caller of the ability to recover, test, or re-raise. Raise a descriptive `PredictabilityEngine::Error` (or a specific subclass) instead and let `exit_on_failure?` in the CLI class handle the exit.
