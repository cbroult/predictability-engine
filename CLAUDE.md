...
- **Commit and push when done**: once a task is complete and `bundle exec rake verify` passes (RSpec + Cucumber + RuboCop + jscpd + bundler-audit all green), commit the changes with a Conventional Commit message and push to the remote. Do not wait to be asked.
